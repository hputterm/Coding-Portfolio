...
def grad_U(Ui, Yij, Vj, Ai, Bj, reg, eta):
    ...
    """
    gradient = reg * Ui
    gradient -= Vj*(Yij - (np.dot(Ui, Vj)+Ai+Bj))
    return eta * gradient
...
def grad_V(Vj, Yij, Ui, Ai, Bj, reg, eta):
    ...
    """
    gradient = reg * Vj
    gradient -= Ui * (Yij - (np.dot(Ui, Vj)+Ai+Bj))
    return eta * gradient
...
def grad_A(Vj, Yij, Ui, Ai, Bj, reg, eta):
    ...
    respect to Vj multiplied by eta.
    """
    gradient = - (Yij - (np.dot(Ui, Vj)+Ai+Bj))
    return eta * gradient
...
def get_err(U, V, Y, A, B, reg=0.0):
    ...
    for training_point in Y:
        ...
        Ai = A[i]
        Bj = B[j]
        err += .5*(np.dot(Ui, Vj)+Ai+Bj-Y_ij)**2
    if reg != 0:
...
def train_model(M, N, K, eta, reg, Y, eps=0.0001, max_epochs=300):
    ...
    A = np.random.uniform(low = -.5, high = .5, size = (M))
    B = np.random.uniform(low = -.5, high = .5, size = (N))
    np.random.shuffle(Y)
    current_error = get_err(U,V,Y,A,B, reg = reg)
    error_initial = 0
    ...
    for epoch in range(max_epochs):
        ...
        for training_point in Y:
            ...
            gradient_U = grad_U(Ui, Y_ij, Vj, Ai, Bj, reg, eta)
            gradient_V = grad_V(Vj, Y_ij, Ui, Ai, Bj, reg, eta)
            gradient_A = grad_A(Vj, Y_ij, Ui, Ai, Bj, reg, eta)
            gradient_B = grad_B(Vj, Y_ij, Ui, Ai, Bj, reg, eta)
        ...
        if(epoch == 0):
            error_initial = current_error - get_err(U,V,Y,A,B, reg = reg)
        ...
        np.random.shuffle(Y)
    return (U,V,A,B,get_err(U,V,Y,A,B))
